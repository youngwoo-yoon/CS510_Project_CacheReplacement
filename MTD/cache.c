	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == FIFO ? "FIFO"
	  : cp->policy == BASELINE ? "BASELINE" //By HC
	  : (abort(), ""));
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  //by H.C: Baseline Parameter
    unsigned int LRU_behavior_set = 1;  //how many times can LRU line be survived? If we give two time of chance to LRU, we just change this value to two.
	unsigned int isNotFull = 0;
  ...
  case Random:
    ...
    break;
  case BASELINE: //By HC
	//Handling a chain that is not full
	...
	if (isNotFull == 1){ 
	/*
		if (set==32){//To be deleted
			printf("===============This chain is not full yet: Unsorted=============\n");			
			printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
			int ct = 0;
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				printf("%d th block - Tag: %d\n",ct, blk->tag);
				ct++;
			}
		}//To be deleted
	*/
	...
	/*	
		if (set==32){//To be deleted
			printf("===============This chain is not full yet: Sorted=============\n");			
			printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
			int ct = 0;
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				printf("%d th block - Tag: %d\n",ct, blk->tag);
	...
	*/	
		...
		//Identifying max_dist block (except for PRE-LRU and MRU)
		unsigned int MRU_loc = 0;
		unsigned int pre_LRU_loc = cp->assoc - 2;
		unsigned int LRU_loc = cp->assoc - 1;
		
		unsigned int base_dist = abs(cp->sets[set].way_tail->tag - tag);
	
		struct cache_blk_t *max_dist_cache_line;
		unsigned int max_dist = 0;
					
		int count = 0;
		for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
			if (count == MRU_loc || count == pre_LRU_loc || count == LRU_loc){
				count++;
				continue;
			} else {
				if (abs(blk->tag - tag) >= max_dist){
					max_dist_cache_line = blk;
					max_dist = abs(blk->tag - tag);
				}
				count++;
			}			
		}
		...
		/*
		if(set==32){ //To be deleted
			printf("===============MAX distance (<--This) Chain vs Base_dist=============\n");
			printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
			printf("Max Block Tag: %d, Max_Dist: %d ||| base_dist: %d\n", max_dist_cache_line->tag, max_dist,base_dist);;
			int cct = 0;
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				printf("%d th block - Tag: %d LRU_Behavior_bit: %d" ,cct, blk->tag, blk->LRU_behavior);
				if(blk->tag == max_dist_cache_line->tag){
					printf(" <--- This");
				}
				printf("\n");
				cct++;
		...
		*/	
		...
		//Comparing max_dist with base_dist
		if(base_dist >= max_dist){
			...
			/*
			if(set==32){
				printf("=============== Base dist >= Max_dist: RUNNING LRU =============\n");
				printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
				printf("Max Block Tag: %d, Max_Dist: %d ||| base_dist: %d\n", max_dist_cache_line->tag, max_dist,base_dist);
				int ct = 0;
				for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
					printf("%d th block - Tag: %d LRU_Behavior_bit: %d" ,ct, blk->tag, blk->LRU_behavior);
					if(blk->tag == max_dist_cache_line->tag){
						printf(" <--- This");
					}
					printf("\n");
			...
			*/
			...
			//cp->sets[set].way_tail->LRU_behavior++;	

			struct cache_blk_t *proceed = max_dist_cache_line->way_prev;
			struct cache_blk_t *middle = cp->sets[set].way_tail;				
			struct cache_blk_t *follow = max_dist_cache_line->way_next;

			struct cache_blk_t *pre_LRU_pointer = cp->sets[set].way_tail->way_prev;
			pre_LRU_pointer->way_next = max_dist_cache_line;
			max_dist_cache_line->way_prev = pre_LRU_pointer;
			max_dist_cache_line->way_next = NULL;
			cp->sets[set].way_tail = max_dist_cache_line;
			
			proceed->way_next = middle;
			middle->way_prev = proceed;
			
			follow->way_prev = middle;
			middle->way_next = follow;	
			
			/*
			if(set==32){
				printf("===============Base dist < Max_dist: MAX_DIST DELETION (TO LRU) =============\n");
				printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
				printf("Max Block Tag: %d, Max_Dist: %d ||| base_dist: %d\n", max_dist_cache_line->tag, max_dist,base_dist);
				int ct = 0;
				for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
  ...
  repl->LRU_behavior = 0; //By HC : for new items, LRU behavior bit is set to 0
  /*
  		if(set==32){
			printf("=======the next=======\n");
			printf("=%s=\n",cp->name);
		...
		for (blk=cp->sets[set].way_head;
		   blk;
		   blk=blk->way_next){
			   printf("%d block line: %d LRU bit = %d\n", count, blk->tag, blk->LRU_behavior);
			   count++;
		   }
		}
  */
 ...
 /* **HIT** */
  ...
  if (blk->way_prev && (cp->policy == LRU || cp->policy == BASELINE))
    {
