	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  //SG: initial size of cluster is half of max distance
  size_half_of_cluster = cp->nsets/4; 
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == CLUSTER ? "CLUSTER" //By SG
	  : (abort(), ""));
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case CLUSTER: //by SG
    {	
	if (cp->sets[set].way_tail->LRU_behavior >= LRU_behavior_set){
		repl = cp->sets[set].way_tail;
		update_way_list(&cp->sets[set], repl, Head);
		...
		if (isNotFull == 1){ 
			...
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
					...
					temp_dist = abs(blk->tag - tag);
					if (blk->LRU_behavior > 0 && temp_dist >= max_dist){
					...
					if(temp_dist < size_half_of_cluster){
        //SG
	blk->LRU_behavior = 0;
}
					count++;
  ...
  repl->LRU_behavior = 0; //SG: for clustered ver. 
//By HC : for new items, LRU behavior bit is set to 0
  /*
  		if(set==32){
			printf("=======the next=======\n");
			printf("=%s=\n",cp->name);
