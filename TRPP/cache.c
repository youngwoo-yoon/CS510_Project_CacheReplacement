	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == FIFO ? "FIFO"
	  : cp->policy == PROPOSED ? "PROPOSED" //By HC
	  : (abort(), ""));
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  //by H.C: PROPOSED method Parameter
  int proposed_method_param = 2;
  ...
  case PROPOSED: //By HC
	...
	if (isNotFull == 1){ 
		...
		*/
		...
		unsigned int inversion_indicator_dist = abs(cp->sets[set].way_head->tag - tag);
		if (inversion_indicator_dist < (cp->nsets*cp->assoc/proposed_method_param)){
			/*			
			if(set==132){
				printf("=============== No Inversion =============\n");
				printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
				printf("Inversion_dicision: %d, |MRU - Request|: %d\n", (cp->nsets*cp->assoc/proposed_method_param), inversion_indicator_dist);
				int ct = 0;
				for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
					printf("%d th block - Tag: %d",ct, blk->tag);
					if(ct == 0){
						printf(" <--- This");
					}
					printf("\n");
			...
			*/
			...
			/*
			if(set==132){
				printf("=============== No Inversion: After SORTING =============\n");
				printf("Set: %d, Address: %d Request Tag: %d \n", set, addr, tag);
				printf("Inversion_dicision: %d, |MRU - Request|: %d\n", (cp->nsets*cp->assoc/proposed_method_param), inversion_indicator_dist);
				int ct = 0;
				for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
			...
			*/
			...
			//Identifying max & min_dist block (except for MRU)
			unsigned int MRU_loc = 0;
		
			struct cache_blk_t *max_dist_cache_line;
			unsigned int max_dist = 0;
			
			struct cache_blk_t *min_dist_cache_line;
			unsigned int min_dist = 5000000000;
			...
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				if (count == MRU_loc){
					...
					if (abs(blk->tag - tag) <= min_dist){
						min_dist_cache_line = blk;
						min_dist = abs(blk->tag - tag);
			...
			if(set==132){ //To be deleted
				...
				for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
					printf("%d th block - Tag: %d",ct, blk->tag);
					if(min_dist_cache_line->tag == blk->tag){
						printf(" <--- MIN");
					}
					if(max_dist_cache_line->tag == blk->tag){
						printf(" <--- MAX");
					}
					printf("\n");
					ct++;
			...
			*/
			...
			repl = max_dist_cache_line;
			update_way_list(&cp->sets[set], repl, Tail);
			repl = min_dist_cache_line;
			update_way_list(&cp->sets[set], repl, Head);
 ...
 /* **HIT** */
  ...
  if (blk->way_prev && (cp->policy == LRU || cp->policy == PROPOSED))
    {
	  /*
	  if(set==132)
	  printf("HIT!!\n");
